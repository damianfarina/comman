...
RSpec.describe Sales::Order, type: :model do
  ...
  describe "associations" do
    ...
    it "has many items" do
      order = create(:sales_order, client: client, items: [
      ...
      expect(order.items).to include(item1, item2)
    end
    ...
    it "destroys associated items when destroyed" do
      ...
      expect { order.destroy }.to change(Sales::Order::Item, :count).by(-2)
    end
  ...
  describe "validations" do
    subject { build(:sales_order, client: client, items: [
      build(:sales_order_item, product: product1, quantity: 1, unit_price: product1.price),
    ]) }
    ...
    it "is invalid without products" do
      subject.items = []
      expect(subject).not_to be_valid
      expect(subject.errors[:items]).to include("no puede estar en blanco")
    end
  ...
  describe "#can_confirm?" do
    ...
    context "when order is a quote" do
      it "returns true if it has confirmable items with valid prices" do
        create(:sales_order_item, order: order, product: product1, quantity: 1, status: "quote", unit_price: product1.price)
      ...
      it "returns false if confirmable items have no effective price" do
        create(:sales_order_item, order: order, product: product1, quantity: 1, status: "quote", unit_price: nil)
        allow_any_instance_of(Sales::Order::Item).to receive(:effective_unit_price).and_return(nil)
        expect(order.can_confirm?).to be false
      end
    end
  ...
  describe "#confirm!" do
    let!(:order) { create(:sales_order, client: client, status: "quote", client_discount_percentage: 10.0, items: [
    ...
    context "when order cannot be confirmed" do
      ...
      it "returns false and adds a combined error message" do
        ...
        base_error_start = I18n.t("activerecord.errors.models.sales/order.attributes.base.confirmation_failed", details: "")
        precondition_error = I18n.t("activerecord.errors.models.sales/order.not_confirmable")
    ...
    context "when an item fails to confirm" do
      before do
        allow(order).to receive(:can_confirm?).and_return(true)
        allow_any_instance_of(Sales::Order::Item).to receive(:confirm!).and_raise(ActiveRecord::RecordInvalid.new(item1))
      end

      it "rolls back the transaction and adds the correct I18n error" do
        expect(order.confirm!).to be false
        expect(order.reload.status).to eq("quote")
        expect(order.errors[:base].first).to start_with(I18n.t("activerecord.errors.models.sales/order.attributes.base.confirmation_failed", details: "").strip)
      end
    end
  end
  ...
  describe "#cancel!" do
    ...
    context "when order cannot be canceled" do
      ...
      it "returns false and adds a combined error message" do
        ...
        precondition_error = I18n.t("activerecord.errors.models.sales/order.not_cancellable")
        base_error_start = I18n.t("activerecord.errors.models.sales/order.attributes.base.cancellation_failed", details: "")
        final_error = order.errors[:base].first
        expect(final_error).to start_with(base_error_start.strip)
        expect(final_error).to include(precondition_error)
      end
  ...
  describe "#fulfill!" do
    ...
    let!(:item_in_progress) { order.items.first }
    ...
    context "when order cannot be fulfilled" do
      ...
      it "returns false and adds a combined error message" do
        ...
        base_error_start = I18n.t("activerecord.errors.models.sales/order.attributes.base.fulfillment_failed", details: "")
        precondition_error = I18n.t("activerecord.errors.models.sales/order.not_fulfillable")
        final_error = order.errors[:base].first
        expect(final_error).to start_with(base_error_start.strip)
        expect(final_error).to include(precondition_error)
      end
  ...
  describe "#subtotal_before_order_discount" do
    it "returns the sum of subtotals for not-canceled items" do
      order = create(:sales_order, client: client, status: "confirmed", items: [
  ...
  describe "#subtotal_after_order_discount" do
    it "returns subtotal_before_order_discount minus client_discount_value" do
      order = create(:sales_order, client: client, status: "confirmed", client_discount_percentage: 10.0, items: [
  ...
  describe "#client_discount_value" do
    it "returns 0 if client_discount_percentage is blank or 0" do
      order = build(:sales_order, client: client, client_discount_percentage: nil, items: [ build(:sales_order_item, product: product1, quantity: 1, status: "in_progress", unit_price: 100) ])
    ...
    it "returns the correct discount value if present and > 0" do
      order = build(:sales_order, client: client, client_discount_percentage: 25, items: [ build(:sales_order_item, product: product1, quantity: 2, status: "in_progress", unit_price: 100) ])
  ...
  describe "#cash_discount_value" do
    it "returns 0 if cash_discount_percentage is blank or 0" do
      order = build(:sales_order, client: client, cash_discount_percentage: nil, client_discount_percentage: 0, items: [ build(:sales_order_item, product: product1, quantity: 1, status: "in_progress", unit_price: 100) ])
    ...
    it "returns the correct cash discount value if present and > 0" do
      order = build(:sales_order, client: client, client_discount_percentage: 10, cash_discount_percentage: 5, items: [ build(:sales_order_item, product: product1, quantity: 2, status: "in_progress", unit_price: 100) ])
      subtotal = 200

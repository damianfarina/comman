...
RSpec.describe "Sales::SalesOrders::Completes", type: :request do
  ...
  describe "POST /create" do
    context "with valid status" do
      it "successfully changes the sales order item status" do
        post complete_sales_sales_order_sales_order_item_path(sales_order, sales_order_item)

        expect(response).to have_http_status(:found)
        expect(response).to redirect_to(sales_sales_order_path(sales_order))

        expect(sales_order_item.reload.status).to eq("ready")
      end
    ...
    context "when sales order does not exist" do
      it "returns not found" do
        post complete_sales_sales_order_sales_order_item_path(99999, sales_order_item)
    ...
    context "when sales order item does not exist" do
      it "returns not found" do
        post complete_sales_sales_order_sales_order_item_path(sales_order, 99999)
        expect(response).to be_not_found
      end
    end
  end

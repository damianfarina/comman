...
RSpec.describe "Sales::Orders::Items::Splits", type: :request do
  ...
  describe "GET /new" do
    it "renders the new split form" do
      get split_sales_order_item_path(sales_order, sales_order_item)
    ...
    context "when sales order does not exist" do
      it "returns 404 status" do
        get split_sales_order_item_path(99999, sales_order_item)
    ...
    context "when sales order item does not exist" do
      it "returns 404 status" do
        get split_sales_order_item_path(sales_order, 99999)
        expect(response).to be_not_found
      end
    end
  end
  ...
  describe "POST /create" do
    ...
    context "with valid parameters" do
      it "successfully splits the sales order item" do
        expect {
          post split_sales_order_item_path(sales_order, sales_order_item), params: split_params
        }.to change { sales_order.items.count }.by(1)
        ...
        sales_order.reload
        new_item = sales_order.items.last
      ...
      context "when requesting turbo_stream format" do
        it "responds with turbo_stream" do
          post split_sales_order_item_path(sales_order, sales_order_item),
               params: split_params,
    ...
    context "when sales order does not exist" do
      it "returns not found" do
        post split_sales_order_item_path(99999, sales_order_item), params: split_params
        expect(response).to be_not_found
      end
    end
    ...
    context "when sales order item does not exist" do
      it "returns not found" do
        post split_sales_order_item_path(sales_order, 99999), params: split_params
        expect(response).to be_not_found
      end
    end
    ...
    context "without required parameters" do
      it "returns bad request" do
        post split_sales_order_item_path(sales_order, sales_order_item), params: {}

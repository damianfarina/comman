...
RSpec.describe "Sales::Orders::Items::UndoStatuses", type: :request do
  ...
  describe "POST /create" do
    context "with valid status" do
      it "successfully undoes the sales order item status" do
        post undo_status_sales_order_item_path(sales_order, sales_order_item)

        expect(response).to have_http_status(:found)
        expect(response).to redirect_to(sales_order_path(sales_order))

        expect(sales_order_item.reload.status).to eq("confirmed")
      end
      ...
      context "when back_to parameter is present" do
        it "redirects to specified URL in turbo_stream format" do
          back_url = sales_root_path

          post undo_status_sales_order_item_path(sales_order, sales_order_item),
               params: { back_to: back_url },
               headers: { 'Accept' => 'text/vnd.turbo-stream.html' }
    ...
    context "with invalid status" do
      before do
        sales_order_item.update(status: "confirmed")
      end
    ...
    context "when sales order does not exist" do
      it "returns not found" do
        post undo_status_sales_order_item_path(99999, sales_order_item)
    ...
    context "when sales order item does not exist" do
      it "returns not found" do
        post undo_status_sales_order_item_path(sales_order, 99999)
        expect(response).to be_not_found
      end
    end
  end

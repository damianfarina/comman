...
RSpec.describe "Sales::Orders::Cancels", type: :request do
  ...
  describe "POST /create" do
    context "with valid status" do
      it "successfully changes the sales order item status" do
        post cancel_sales_order_sales_order_item_path(sales_order, sales_order_item)
      ...
      # context "when sales order item is ready" do
      #   before do
      #     sales_order_item.update(status: "ready")
      #   end

      #   it "returns products stock" do
      #     expect(sales_order_item.product.stock).to eq(0)
      #     post cancel_sales_order_sales_order_item_path(sales_order, sales_order_item)
      #     expect(sales_order_item.product.stock).to eq(sales_order_item.quantity)
      #   end
      # end
    end

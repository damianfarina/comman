module ApplicationHelper
  ...
  def status_container_for(status, opts = {})
    color_classes = {
      "quote"       => "text-white text-shadow-gray bg-gray-100",
      "confirmed"   => "text-blue-700 text-shadow-white-700 bg-light-blue",
      "in_progress" => "text-white text-gray bg-yellow",
      "ready"       => "text-white text-shadow-blue-900 bg-blue-100",
      "delivered"   => "text-green-900 text-shadow-white bg-light-green-300",
      "fulfilled"   => "text-white text-shadow-green-900 bg-green",
      "canceled"   => "text-white text-shadow-red bg-light-red",
    }
    content_tag(:span, class: "inline-block px-1.5 py-1 rounded-md text-shadow-xs shadow-sm #{color_classes[status.to_s]} #{opts[:class]}") do

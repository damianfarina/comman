class MakingOrder < ActiveRecord::Base
  ...
  validates :making_order_formula,
    ...
    :mixer_capacity,
    presence: true
  validates :mixer_capacity,
    numericality: { greater_than: 1},
    if: 'self.mixer_capacity.present?'
...
private
  ...
  def build_making_order_formula_if_needed
    self.build_making_order_formula(
      formula_id: making_order_items.first.product.formula_id
    ) if need_formula_and_is_available?
  end

  def calculate_total_weight
    self.total_weight = making_order_items
